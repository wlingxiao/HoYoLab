...
public class GenshinExchangeCdkeyRequest(string uid, string cdkey, Region region) : IRequest
{
    public HttpMethod Method => HttpMethod.Get;

    public Uri RequestUri() => new Uri(
        $"https://sg-hk4e-api.hoyoverse.com/common/apicdkey/api/webExchangeCdkey?uid={uid}&region={region.ToGenshinRegion()}&lang=zh-tw&cdkey={cdkey}&game_biz=hk4e_global&sLangKey=en-us");
}
