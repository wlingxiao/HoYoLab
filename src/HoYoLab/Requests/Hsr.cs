...
public class HsrExchangeCdkeyRequest(string uid, string cdkey, Region region) : IRequest
{
    public HttpMethod Method => HttpMethod.Get;

    public Uri RequestUri() => new Uri(
        $"https://sg-hkrpg-api.hoyoverse.com/common/apicdkey/api/webExchangeCdkey?t={DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}&lang=ja&game_biz=hkrpg_global&uid={uid}&region={region.ToHsrRegion()}&cdkey={cdkey}");
}
